export const DATA_FILES = {
  ...
  qaResearch: '/data/green_fund_qa_research.json',
  qaOverrides: '/data/green_fund_qa_overrides.json'
};
...
// ponytail: these two files are generated by scripts/research-unmapped.mjs / the QA export
// button and may not exist yet, so a missing file is a normal empty state, not an error.
...
export async function loadDashboardData() {
  const [
    ...
    fetchJsonOptional(DATA_FILES.qaResearch, { projects: {} }),
    fetchJsonOptional(DATA_FILES.qaOverrides, {})
  ]);
  ...
  return {
    ...
    qaResearch: qaResearch?.projects || {},
    qaOverridesBase: qaOverrides || {}
  };
}

// Folds QA overrides (committed baseline + this browser's unsaved edits) into the dataset:
// projects assigned an atoll/island move from "unmapped" into normal mapped spending so
// they show up on the map and in totals immediately, without touching the source CSVs.
export function applyQaOverrides(data, overrides) {
  const entries = Object.entries(overrides || {}).filter(([, o]) => o?.status === 'assigned' && o.join_key);
  if (!entries.length) return data;
  const assignedCodes = new Set(entries.map(([code]) => code));
  const overrideByCode = new Map(entries);

  const movedMonthly = data.unmappedMonthly
    .filter((row) => assignedCodes.has(row.project_code))
    .map((row) => {
      const o = overrideByCode.get(row.project_code);
      return {
        ...row,
        allocated_amount_mvr: row.amount_mvr,
        parsed_join_key: o.join_key,
        parsed_atoll: o.atoll,
        parsed_island: o.island,
        spatial_scope_clean: 'qa_assigned'
      };
    });

  return {
    ...data,
    monthlyLocations: [...data.monthlyLocations, ...movedMonthly],
    unmappedMonthly: data.unmappedMonthly.filter((row) => !assignedCodes.has(row.project_code)),
    unmappedProjects: data.unmappedProjects.filter((row) => !assignedCodes.has(row.project_code))
  };
}
